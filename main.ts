...
let variable = scene.createRenderable(zLayer1, (image1: Image, camera: scene.Camera) => {
    ...
    helpers.imageBlit(tempImg, 0, 0, Math.ceil(160 / blurSize1), Math.ceil(120 / blurSize1), screenClone, 0, 0, 160, 120, true, false)
        helpers.imageBlit(screenClone, 0, 0, 160, 120, tempImg, (tempImg.width * blurSize1 - 160) / 2, (tempImg.width * blurSize1 - 120) / 2, tempImg.width - (tempImg.width * blurSize1 - 160) / 2, tempImg.height - (tempImg.width * blurSize1 - 120) / 2, true, false)
    }
    if (zoomSize1 != 1) {
